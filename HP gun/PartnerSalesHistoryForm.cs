...
namespace HP_gun
{
    public partial class PartnerSalesHistoryForm : Form
    {
        ...
        private void LoadSalesHistory()
        {
            ...
            try
            {
                ...
                using (var connection = new NpgsqlConnection(connectionString))
                {
                    ...
                    string query = @"SELECT  p.name , pp.quantity, pp.start_date 
                                    FROM partner_products pp
                                    JOIN product p ON pp.product_id_fk = p.product_id
                                    WHERE pp.partner_id_fk = @partnerId
                    ...
                    using (var cmd = new NpgsqlCommand(query, connection))
                    {
                        ...
                        using (var reader = cmd.ExecuteReader())
                        {
                            ...
                            while (reader.Read())
                            {
                                ...
                                flowLayoutPanel1.Controls.Add(salePanel);
                            }
                        }
                    }
                }
        ...
        private Panel CreateSalePanel(NpgsqlDataReader reader)
        {
            ...
            var saleDate = Convert.ToDateTime(reader["start_date"]);
